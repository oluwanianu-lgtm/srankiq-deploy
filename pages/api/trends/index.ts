  ...
  try {
    ...
    if (platform === 'YouTube') {
      ...
      const ytRes = await fetch(
        `https://www.googleapis.com/youtube/v3/videos?part=snippet,statistics&chart=mostPopular&regionCode=US&maxResults=12&key=${ytKey}`
      )
      ...
      if (ytData.items?.length > 0) {
        ...
        return res.status(200).json({ trends, summary: 'Top trending videos right now' })
      }
    }
