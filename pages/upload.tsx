...
function UploadPage() {
  ...
  const generateAI = async () => {
    ...
    try {
      const [titlesRes, hashRes] = await Promise.all([
        apiPost('/api/ai/titles', { topic: form.title, platform: activePlt.name, style: 'viral', keywords: [] }),
        apiPost('/api/ai/hashtags', { topic: form.title, platform: activePlt.name, count: 20 }),
      ])
  ...
  const runSeoScan = async () => {
    ...
    setSeo(null)
    try {
      const res = await apiPost('/api/ai/seo', {
        title: form.title,
